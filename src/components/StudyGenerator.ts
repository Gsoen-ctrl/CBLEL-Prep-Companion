...
export async function generateQuestion(): Promise<StudyQuestion> {
  ...
  if (isClassification) {
    ...
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (classType === "DDC") {
          if (line.startsWith("###")) {
            ...
            if (isEasy) {
              ...
              if (match) items.push({ code: match[1].trim(), description: match[2].trim() });
            }
        ...
        if (!isEasy || (isEasy && !inEasyHeader)) {
           if (line.startsWith("|")) {
             ...
             if (cols.length >= 2 && !cols[0].includes("---") && cols[0] !== "Number" && cols[0] !== "Class") {
      ...
      const finalOptions = options.sort(() => 0.5 - Math.random()).map((opt, i) => ({
...
async function generateStandardQuestion(): Promise<StudyQuestion> {
  const subject = SUBJECT_SHORTS[Math.floor(Math.random() * SUBJECT_SHORTS.length)];
  try {
    ...
    for (const line of lines) {
      ...
      if (qMatchOld || qMatchNew) {
        if (current && current.stem && current.options.length > 0) questions.push(current);
        ...
        const letter = oMatchOld ? oMatchOld[2].toUpperCase() : String.fromCharCode(65 + current.options.length);
